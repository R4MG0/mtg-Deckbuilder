...
def shuffle_library(deck_name):
    ...
    cur.execute(
        f"SELECT COUNT(*) FROM library WHERE deck_name = ?", (deck_name,))
    ...
    for i, card_id in enumerate(card_ids, 1):
        cur.execute(
            f"UPDATE library SET id = ? WHERE id = ? AND deck_name = ?", (i, card_id, deck_name))
...
def draw_card(deck_name):
    ...
    cur.execute(
        f"SELECT id, name, img_url FROM library WHERE deck_name = ? ORDER BY id ASC LIMIT 1", (deck_name,))
    card = cur.fetchone()
    if card is not None:
        cur.execute(f"DELETE FROM library WHERE id = ?", (card[0],))
        cur.execute(f"INSERT INTO hand (name, img_url, deck_name, player_name) VALUES (?, ?, ?, ?)",
                    (card[1], card[2], deck_name, player_name))
...
def cast_card(player_name, card_name):
    ...
    cur.execute(f"SELECT id, name, img_url FROM hand WHERE deck_name = ? AND name = ?",
                (deck_name, card_name))
    card = cur.fetchone()
    if card is not None:
        cur.execute(
            f"DELETE FROM hand WHERE id = ? AND controller = ?", (card[0], ))
        cur.execute(f"INSERT INTO battlefields_{player_name} (name, img_url, deck_name, player_name) VALUES (?, ?, ?, ?)", (
            card[1], card[2], deck_name, player_name))
...
def move_permanent(from_pos, to, card_name):
    ...
    card = cur.fetchone()
    cur.execute(f'INSERT INTO ? VALUES (?,?,?,?,?)',
                (to, card[0],  card[1], card[2], card[3], card[4]))
...
def destroy_permanent(player_name, deck_name, card_name):
    ...
    cur.execute(
        f"SELECT id, name, img_url FROM battlefields_{player_name} WHERE deck_name = ? AND name = ?", (deck_name, card_name))
    card = cur.fetchone()
    if card is not None:
        cur.execute(
            f"DELETE FROM battlefields_{player_name} WHERE id = ?", (card[0],))
        cur.execute(f"INSERT INTO graveyards_{player_name} (name, img_url, deck_name, player_name) VALUES (?, ?, ?, ?)", (
            card[1], card[2], deck_name, player_name))
...
def exile_card(player_name, card_id):
    ...
    if not card_data:
        c.execute(
            f"SELECT * FROM battlefields_{player_name} WHERE id={card_id}")
        card_data = c.fetchone()
    ...
    if card_data:
        ...
        c.execute(
            f"INSERT INTO exiles_{player_name} (name, img_url, deck_name, player_name) VALUES ('{card_name}', '{img_url}', '{deck_name}', '{player_name}')")
        conn.commit()
        conn.close()
